...
public class SeResourceDao {
    ...
    public SeResource findOne(Long resourceId) {
        final String sql = "select id, name, type, url, permission, parent_id, parent_ids, available,business_type from pf_se_resource where id=?";
    ...
    public List<SeResource> findByBusinessType(String type) {
        ...
        final String sql = "select id, name, type, url, permission, parent_id, parent_ids, available,business_type from pf_se_resource where parent_id > 2 and parent_id != 11 and business_type in("+conType+") order by concat(parent_ids, id) desc";
